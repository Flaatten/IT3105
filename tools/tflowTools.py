...
def gen_initialized_session(dir='probeview'):
    sess = tf.Session()
    # Create a probe stream and attach to the session
    sess.probe_stream = viewprep(sess, dir=dir)
...
def close_session(sess, view=True):
    ...
    sess.close()
    if view:
        fireup_tensorboard(sess.viewdir)
...
def viewprep(session, dir='probeview', flush=120, queue=10):
    # Without this, the directory fills up with unusable files
    clear_tensorflow_log(dir)
...
def gen_line_array(dims, indices, line_item=1, background=0, columns=False, bias=1.0):
    ...
    if columns:
        a = a.reshape(list(reversed(dims)))
    for row in indices:
...
def gen_vector_count_cases(num, size, drange=(0, 1), random=True, poptarg=True):
    if random:
        feature_vectors = gen_random_density_vectors(
            num, size, density_range=drange)
    else:
...
def gen_all_binary_count_cases(size, poptarg=True): return gen_vector_count_cases(
    None, size, random=False, poptarg=poptarg)
...
def gen_random_line_cases(num_cases, dims, min_lines=1, min_opens=1, bias=1.0, mode='classify',
    ...
    def gen_case():
        # Randomly choose to use a row or a column
        label = np.random.choice([0, 1])
        features = gen_features(label)
        if flat:
            features = features.flatten().tolist()
        # It's a classification task, so use 2 neurons, one for each class (horiz, or vert)
        if mode == 'classify':
            target = [0] * 2
            target[label] = 1
        elif mode == 'auto':
...
def gen_random_pieces(chunk_size, num_pieces):
    if num_pieces == 1:
        ...
        cut_points = list(NPR.choice(range(1, chunk_size),
                                     num_pieces - 1, replace=False))
        lastloc = 0
        pieces = []
...
def gen_segmented_vector_cases(vectorlen, count, minsegs, maxsegs, poptargs=True):
    ...
    for c in range(count):
        ...
        case = [v, int_to_one_hot(
            numsegs, maxsegs - minsegs + 1)] if poptargs else [v, numsegs]
        cases.append(case)
    return cases
...
def segment_count(vect, onval=1, offval=0):
    ...
    count = 0
    for elem in vect:
        if elem == onval and lastval == offval:
            count += 1
        lastval = elem
    return count
...
def show_results(grabbed_vals, grabbed_vars=None, dir='probeview'):
    showvars(grabbed_vals, names=[
             x.name for x in grabbed_vars], msg="The Grabbed Variables:")
...
def plot_training_history(error_hist, validation_hist=[], xtitle="Epoch", ytitle="Error", title="History", fig=True):
    ...
    if fig:
        PLT.figure()
    if len(error_hist) > 0:
        simple_plot([p[1] for p in error_hist], [p[0]
                                                 for p in error_hist], xtitle=xtitle, ytitle=ytitle, title=title)
        PLT.hold(True)
    if len(validation_hist) > 0:
        simple_plot([p[1] for p in validation_hist], [p[0]
                                                      for p in validation_hist])
    PLT.ioff()
...
def hinton_plot(matrix, maxval=None, maxsize=1, fig=None, trans=True, scale=True, title='Hinton plot',
    ...
    for (x, y), val in np.ndenumerate(matrix):
        ...
        if scale:
            size = max(0.01, np.sqrt(
                min(maxsize, maxsize * np.abs(val) / maxval)))
        else:
...
def display_matrix(matrix, fig=None, trans=True, scale=True, title='Matrix', tform='{:.3f}', tsize=12,
    ...
    for (x, y), val in np.ndenumerate(matrix):
        ...
        blob = PLT.Rectangle(
            botleft, 1, 1, facecolor='white', edgecolor='white')
        axes.add_patch(blob)
...
def pca(features, target_size, bias=True, rowvar=False):
    farray = features if isinstance(
        features, np.ndarray) else np.array(features)
    # rowvar=False => each var's values are in a COLUMN.
    cov_mat = np.cov(farray, rowvar=rowvar, bias=bias)
...
def gen_dim_reduced_data(feature_array, target_size, eigen_values, eigen_vectors):
    ...
    # Sorts tuples by their first element = abs(eigenvalue)
